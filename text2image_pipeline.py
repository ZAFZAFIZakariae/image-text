...
def generate_image(
    ...
    workflow:
        Controls whether to run only the base pipeline ("base-only"), force the
        refiner stage when available ("base+refiner"), or automatically pick
        based on the model configuration ("auto").
    ...
    workflow_normalised = workflow.strip().lower()
    if workflow_normalised not in {"auto", "base-only", "base+refiner"}:
        raise ValueError(
            "workflow must be one of 'auto', 'base-only', or 'base+refiner'"
        )
    ...
    if workflow_normalised == "base-only":
        refiner_pipeline = None
    elif workflow_normalised == "base+refiner" and refiner_pipeline is None:
        raise RuntimeError(
            "Requested 'base+refiner' workflow but the selected model does not "
            "define a refiner pipeline."
        )
